...
from helpers.mocks import create_mock_ingredient
...
class TestBurger:
    ...
    def test_move_ingredient(self, burger):
        ing1 = create_mock_ingredient(name="A")
        ing2 = create_mock_ingredient(name="B")
    ...
    def test_get_price(self, burger, mock_bun, bun_price, ingredients_prices, expected_price):
        mock_bun.get_price.return_value = bun_price
        burger.set_buns(mock_bun)
        for price in ingredients_prices:
            ingredient = create_mock_ingredient(price=price)
            burger.add_ingredient(ingredient)
    ...
    @pytest.mark.parametrize("bun_name, ingredient_data, expected_receipt", [
        ...
        (
            "White Bun",
            [(INGREDIENT_TYPE_FILLING, "Cheese", 25), (INGREDIENT_TYPE_SAUCE, "Mayo", 25)],
            "(==== White Bun ====)\n= filling Cheese =\n= sauce Mayo =\n(==== White Bun ====)\n\nPrice: 150"
        ),
    ])
    def test_get_receipt(self, burger, mock_bun, bun_name, ingredient_data, expected_receipt):
        mock_bun.get_name.return_value = bun_name
        mock_bun.get_price.return_value = 50
        burger.set_buns(mock_bun)
        for ing_type, name, price in ingredient_data:
            ingredient = create_mock_ingredient(name=name, price=price, ing_type=ing_type)
            burger.add_ingredient(ingredient)
        assert burger.get_receipt() == expected_receipt

    def test_get_receipt_no_bun(self, burger):
        ingredient = create_mock_ingredient(name="Ketchup", price=50, ing_type=INGREDIENT_TYPE_SAUCE)
        burger.add_ingredient(ingredient)
        with pytest.raises(AttributeError):
            burger.get_receipt()
